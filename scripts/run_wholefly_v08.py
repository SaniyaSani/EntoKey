#!/usr/bin/env python3
"""TaxaLens v0.8: plan → DINOv3 whole-fly embeddings → hierarchical heads → retrieval."""
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/wholefly_foundation_v08.json")
    parser.add_argument("--work-dir", default="data/wholefly_v08")
    parser.add_argument("--model-dir", default="models_wholefly_v08")
    ...
    if embedding.get("segmentation_required") is True:
        raise SystemExit("v0.8 default profile must not require anatomy segmentation")
    if embedding.get("strategy", "whole_image_only") == "whole_image_only" and int(embedding.get("tile_grid", 1)) != 1:
        raise SystemExit("whole_image_only baseline requires tile_grid=1; use a separate ablation config for tiles")
